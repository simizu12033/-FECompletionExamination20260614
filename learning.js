...
const isReview=()=>learningState.phase!=="exam";
function visible(q){
...
function renderLearning(){
  ...
  QUESTIONS.forEach(q=>{
    ...
    node.querySelectorAll(".answer-input input").forEach(input=>{
      input.name=`answer-${q.n}`;input.checked=learningState.answers[q.n]===input.value;
      input.onchange=()=>{
        learningState.answers[q.n]=input.value;saveLearning();
        updateLearningHeader();renderLearningMap();updatePhasePanel();
      };
    });
    if(isReview()){
      const correct=learningState.answers[q.n]===q.answer;
      const line=node.querySelector(".result-line");line.className=`result-line ${correct?"correct":"wrong"}`;
      line.textContent=correct?`正解：あなたの回答 ${learningState.answers[q.n]}`:`不正解：あなたの回答 ${learningState.answers[q.n]} ／ 正答 ${q.answer}`;
    }
...
function updatePhasePanel(){
  ...
  qs("#reviewToolbar").hidden=!review;
  document.querySelectorAll("[data-exam-controls]").forEach(el=>el.hidden=review);
  qs("#listTitle").textContent=review?"採点結果と解説":"試験問題";
  qs("#phaseMessage").textContent=review?"採点済みです。解説を読み、理解できた問題にチェックを入れてください。":`${answeredCount()}問回答済み。60問すべて回答すると採点できます。`;
  document.querySelectorAll('[data-action="submit"]').forEach(button=>button.disabled=answeredCount()!==60);
  document.querySelectorAll("[data-exam-message]").forEach(message=>{
    message.textContent=answeredCount()===60?"全60問に回答しました。採点すると解答は変更できません。":`現在 ${answeredCount()} / 60問回答済みです。`;
  });
  const summary=qs("#scoreSummary");summary.hidden=!review;
  qs("#answerLegend").hidden=!review;
  if(review)summary.innerHTML=`<div><strong>${score()}</strong><span>正解</span></div><div><strong>${60-score()}</strong><span>不正解</span></div><div><strong>${learningState.understood.size}</strong><span>理解済み</span></div>`;
  document.querySelectorAll("#phaseSteps [data-step]").forEach(el=>{
    el.classList.remove("active","finished");const step=el.dataset.step;
    if(!review&&step==="exam")el.classList.add("active");
    if(review&&(step==="exam"||step==="score"))el.classList.add("finished");
    if(review&&!complete&&step==="review")el.classList.add("active");
  ...
  if(review){ensureFirstReport();renderFirstReport()}else qs("#learningReport").hidden=true;
}
function renderLearningMap(){
  qs("#questionMap").innerHTML=QUESTIONS.map(q=>{
    let cls="";
    if(!isReview()&&learningState.answers[q.n])cls="answered";
    if(isReview())cls=learningState.answers[q.n]===q.answer?"correct":"wrong";
...
function renderFirstReport(){
  ...
  qs("#retestQuestions").innerHTML=report.wrong.length?report.wrong.map(n=>`<a href="#q${n}">問${n}</a>`).join(""):`<span>全問正解でした。60問をもう一度通して定着を確認しましょう。</span>`;
...
function initLearning(){
  ...
  qs("#confirmSubmit").onclick=()=>{
    learningState.phase="review";saveLearning();ensureFirstReport();
