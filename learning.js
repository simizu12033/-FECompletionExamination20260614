...
const currentScore=()=>QUESTIONS.filter(q=>currentAnswer(q)===q.answer).length;
const needsRetry=q=>isReview()&&currentAnswer(q)!==q.answer;
...
function renderLearning(){
  ...
  QUESTIONS.forEach(q=>{
    ...
    if(isReview()){
      const userAnswer=currentAnswer(q);
      const correct=userAnswer===q.answer;
      const line=node.querySelector(".result-line");
      line.className=`result-line ${correct?"correct":"wrong"}`;
      line.textContent=correct
        ?`正解：あなたの回答 ${userAnswer}`
        :`不正解：あなたの回答 ${userAnswer||"未回答"}。正答と解説は隠しています。もう一度選択肢を選んでください。`;
    }
...
function updatePhasePanel(){
  ...
  qs("#phaseMessage").textContent=isReview()
    ?`採点済みです。間違っている問題は選択肢を押して再回答できます。正解するまで正答と解説は表示しません。（未解決 ${unresolved}問）`
  ...
  if(isReview()){
    summary.innerHTML=`<div><strong>${currentScore()}</strong><span>現在の正解</span></div><div><strong>${60-currentScore()}</strong><span>未解決</span></div><div><strong>${learningState.understood.size}</strong><span>理解済み</span></div>`;
  }
...
function renderLearningMap(){
  qs("#questionMap").innerHTML=QUESTIONS.map(q=>{
    ...
    if(isReview())cls=currentAnswer(q)===q.answer?"correct":"wrong";
